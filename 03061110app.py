...
with tab_ocr:
    ...
    if wynik is not None:
        ...
        m2.metric("Bez indeksu (pominięte)", bez_indeksu)
        suma_gotowych = 0.0
        ...
        if bez_indeksu:
            st.warning(
                f"⚠️ **{bez_indeksu}** pozycji nie ma indeksu wewnętrznego i NIE zostanie "
                "wpisanych. Obsłuż je wyżej (usuń / dopasuj / wpisz indeks ręcznie w tabeli), "
                "albo wpisz je w maggo osobno."
            )
...
with tab_pomoc:
    ...
    st.info("🖼️ *TU ZRZUT: strona python.org z żółtym przyciskiem „Download Python”.*")
    ...
    st.info("🖼️ *TU ZRZUT: pierwsze okno instalatora „Czy zainstalować…”, z przyciskiem „Zainstaluj”.*")
    ...
    st.info("🖼️ *TU ZRZUT: czarne okno z pytaniem „Update setting now? [y/N]” — z wpisanym y.*")
    st.info("🖼️ *TU ZRZUT: czarne okno „Add commands directory to your PATH? [y/N]” — z wpisanym y.*")
    st.info("🖼️ *TU ZRZUT: „Install CPython now? [Y/n]” oraz „Installing Python 3.xx”.*")
    ...
    st.info("🖼️ *TU ZRZUT: menu Start z wpisanym „powershell”.*")
    ...
    st.info("🖼️ *TU ZRZUT: PowerShell po obu komendach — „Successfully installed…” i pobrana przeglądarka.*")
    ...
    st.info("🖼️ *TU ZRZUT: faktura w maggo, zakładka „Pozycje”, widoczny przycisk „Dodaj”.*")
    ...
    st.info("🖼️ *TU ZRZUT: okienko „Wybierz plik CSV” z zaznaczonym plikiem pozycje_xxx.csv.*")
