    ...
    try:
        ...
        with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
            for pg in pdf.pages:
                ...
                for top, rzad in linie:
                    rzad = sorted(rzad, key=lambda w: w['x0'])
                    if not rzad:
                        continue
                    pierwszy = rzad[0]
                    if pierwszy['x0'] > 30 or not re.match(r'^\d{1,3}$', pierwszy['text']):
                        continue
                    teksty = [w['text'] for w in rzad]
                    if not any(t in ('szt.', 'kpl.', 'szt', 'kpl', 'kg', 'm', 'l', 'op.', 'para')
                               for t in teksty):
                        continue
                    lp = int(pierwszy['text'])
                    symbol = " ".join(w['text'] for w in rzad
                                      if 28 < w['x0'] < _ALB_X_SYMBOL_END).strip()
