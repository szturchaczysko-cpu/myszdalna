...
with tab_ocr:
    ...
    plik = st.file_uploader("Faktura (XML z KSeF / PDF / JPG / PNG)",
                            type=["xml", "pdf", "jpg", "jpeg", "png", "webp"],
                            accept_multiple_files=False)
    st.caption("💡 Najlepszy odczyt daje **XML z KSeF** (dane dokładne, bez OCR). "
               "PDF i zdjęcia też działają.")
    ...
    if wynik is not None:
        ...
        def _rdzen(s: str) -> str:
            """Indeks do porównań globalnych: same alfanumeryki, wielkie litery.
            '12754AM' / 'MAS 12754' / '12754MASIERO' -> zawierają rdzeń '12754'."""
            import re as _re
            return _re.sub(r"[^A-Z0-9]", "", (s or "").upper())

        # Indeks globalny: rdzeń kodu producenta -> (indeks_glowny, kod_producenta, nazwa)
        # Budujemy raz. Kod producenta jest uniwersalny (ten sam u każdego dostawcy),
        # więc po nim można dopasować pozycję NIEZALEŻNIE od tego, czy znamy dostawcę.
        @st.cache_data(show_spinner=False)
        def _buduj_indeks_globalny(_sygn: str):
            idx = {}
            cache_dokl = lookup._cache.get("po_dostawcy_dokladnie", {})
            for indeksy in cache_dokl.values():
                for wpis in indeksy.values():
                    kp = (wpis.get("kod_producenta") or "").strip()
                    ig = (wpis.get("indeks_glowny") or "").strip()
                    for klucz in (kp, ig):
                        rk = _rdzen(klucz)
                        if rk and rk not in idx:
                            idx[rk] = {
                                "indeks_glowny": ig,
                                "kod_producenta": kp or ig,
                                "nazwa": wpis.get("nazwa", ""),
                                "nazwa_producenta": wpis.get("nazwa_producenta", ""),
                            }
            return idx

        indeks_globalny = _buduj_indeks_globalny(
            lookup._zbudowano_kiedy or str(len(lookup._cache.get("po_dostawcy_dokladnie", {})))
        )

        def dopasuj_globalnie(indeks_z_faktury: str):
            """Szuka pozycji po rdzeniu w CAŁEJ bazie (po kodzie producenta /
            indeksie wewn.). Zwraca wpis albo None. Wymaga rdzenia >=3 znaki."""
            rk = _rdzen(indeks_z_faktury)
            if len(rk) < 3:
                return None
            if rk in indeks_globalny:           # trafienie dokładne po rdzeniu
                return indeks_globalny[rk]
            # trafienie, gdy rdzeń z faktury zawiera się w kluczu albo odwrotnie
            # (np. faktura '12754AM' -> rdzeń '12754AM'; w bazie rdzeń '12754')
            kandydaci = [v for k, v in indeks_globalny.items() if k in rk or rk in k]
            # akceptuj tylko jednoznaczny wynik (jeden indeks wewnętrzny)
            unikalne = {v["indeks_glowny"] for v in kandydaci}
            if len(unikalne) == 1:
                return kandydaci[0]
            return None
        ...
        for p in wynik.pozycje:
            ...
            # 4) zwykły lookup (po dostawcy — najpewniejszy)
            ...
            # 5) dopasowanie GLOBALNE — po kodzie producenta / indeksie wewn. z całej bazy
            #    (kod producenta jest uniwersalny, więc dostawca nie musi się zgadzać)
            g = dopasuj_globalnie(p.indeks)
            if g is not None:
                wiersze.append(_wiersz(p, g["indeks_glowny"], "globalne (kod prod.)",
                                       g.get("kod_producenta", "")))
            else:
        ...
        glob = (df["dopasowanie"] == "globalne (kod prod.)").sum() if len(df) else 0
        ...
        info_bits = [f"dokładne: **{dokladne}**", f"znormalizowane: **{znorm}**",
                     f"globalne: **{glob}**", f"wyuczone/ręczne: **{wyu}**", f"BRAK: **{brak_n}**"]
        if pominiete_czarna:
        ...
        # Do maggo idzie INDEKS DOSTAWCY — dokładnie to, co jest na fakturze
        # (maggo dopasowuje pozycję po swojej stronie po artkIndeks). Pozycja jest
        # "gotowa", gdy ma niepusty indeks dostawcy (czyli praktycznie każda odczytana).
        gotowe = df_edit[df_edit["indeks_dostawcy"].astype(str).str.strip() != ""].copy()
        bez_indeksu = len(df_edit) - len(gotowe)

        # podsumowanie liczbowe
        m1, m2, m3 = st.columns(3)
        m1.metric("Gotowe do wpisania", len(gotowe))
        m2.metric("Bez indeksu dostawcy", bez_indeksu)
        suma_gotowych = 0.0
        ...
        if bez_indeksu:
            st.warning(
                f"⚠️ **{bez_indeksu}** pozycji nie ma indeksu dostawcy i NIE zostanie wpisanych. "
                "Uzupełnij go w kolumnie **„Indeks dostawcy”** w tabeli wyżej."
            )
        st.caption(
            "Do maggo trafia **indeks dostawcy** (to, co na fakturze) — maggo dopasowuje pozycję "
            "po swojej stronie. Kolumny „Indeks wewn.” i „Kod producenta” są tylko poglądowe."
        )
        ...
        if len(gotowe):
            ...
            for _, r in gotowe.iterrows():
                ...
                w.writerow([
                    str(r["indeks_dostawcy"]).strip(),   # <-- INDEKS DOSTAWCY idzie do maggo
            ...
            with kol_b:
                st.caption(f"Plik: **{nazwa_csv}** · do maggo trafia **indeks dostawcy**")
...
with tab_pomoc:
    ...
    st.markdown(
        ...
        "kliknij **dwa razy** plik **`Wpisz_do_maggo.bat`**. Otworzy się czarne okno **oraz "
        "osobne okno Chrome** (specjalnie do wpisywania — nie rusza Twojej zwykłej przeglądarki)."
    )
    st.caption(
    ...
    st.markdown(
        "**Krok 3.5** — W tym **nowo otwartym oknie Chrome**: jeśli to pierwszy raz, zaloguj "
        "się do maggo (to okno ma własny profil, więc zapamięta logowanie na kolejne wpisy). "
        "Następnie wejdź na **tę fakturę** → zakładka **Pozycje** (widoczny przycisk *Dodaj*). "
        "Pracuj w tym oknie normalnie — to zwykły Chrome."
    )
    st.info(
        "💡 To rozwiązuje stary problem z pustym oknem („about:blank”) i przeklejaniem linków. "
        "Teraz po prostu wchodzisz na fakturę w tym oknie Chrome — skrypt sam ją znajdzie."
    )
    st.markdown(
        "**Krok 3.6** — Wróć do **czarnego okna** i naciśnij **Enter**. Skrypt sam odnajdzie "
        "kartę z otwartą fakturą."
    )
    st.markdown(
    ...
    st.markdown(
        ...
        "Okno Chrome do wpisywania możesz zostawić otwarte na kolejne faktury."
    )
